elifeLibrary {
    ...
    node('containers-jenkins-plugin') {
        ...
        stage 'Build images', {
            ...
            try {
                candidateVersion = sh(script: "IMAGE_TAG=${commit} docker-compose -f docker-compose.yml -f docker-compose.ci.yml run sciencebeam-utils ./print_version.sh", returnStdout: true).trim()
        ...
        stage 'Project tests', {
            try {
                sh "IMAGE_TAG=${commit} docker-compose -f docker-compose.yml -f docker-compose.ci.yml run sciencebeam-utils ./project_tests.sh"
            } finally {
                sh 'docker-compose down -v'
            }
        }
